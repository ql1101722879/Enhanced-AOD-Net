...
def populate_train_list(orig_images_path, hazy_images_path):
	...
	# image_list_haze = glob.glob(hazy_images_path + "*.jpg")  FogShips
	image_list_haze = glob.glob(hazy_images_path + "*.jpg") ## NYU
	...
	for image in image_list_haze:
		image = image.split("\\")[-1]
		# key = image.split("_")[0] + "_" + image.split("_")[1] + ".jpg" ## NYU_1数据集
		key = image.split("_")[0] + ".jpg"  # FogShips数据集
		# print(key)
